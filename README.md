...
#### 게시글 작성자 관리
게시글 테이블에는 **작성자명을 직접 저장하지 않고**, 작성자 회원 ID(`member_id`)만 저장합니다.

**장점:**
- ✅ 데이터 중복 제거 (정규화)
- ✅ 회원이 닉네임 변경 시 모든 게시글에 자동 반영
- ✅ 데이터 일관성 보장
- ✅ 저장 공간 효율화

**구현 방식:**
```sql
-- 게시글 조회 시 member 테이블과 JOIN
SELECT b.board_id, b.title, b.content, m.nickname, b.created_at
FROM board b
...
회원 권한을 `role` 컬럼으로 관리합니다.

| role | 설명 |
|:---:|:---|
| `USER` | 일반 사용자 (게시글 작성, 댓글 작성 가능) |
| `ADMIN` | 관리자 (전체 권한) |

**확장 가능한 구조:**
```sql
-- 향후 MODERATOR, PREMIUM 등 추가 가능
ALTER TABLE member ADD COLUMN role VARCHAR(50) DEFAULT 'USER';
```
...
**구현 방식:**
```sql
-- 회원 삭제 (실제로는 deleted = 1로만 변경)
UPDATE member SET deleted = 1, updated_at = NOW() WHERE member_id = ?;

-- 조회 시 삭제된 회원 제외
SELECT * FROM member WHERE deleted = 0;

-- 게시글 조회 시에도 삭제된 회원 제외
SELECT b.*, m.nickname 
FROM board b
JOIN member m ON b.member_id = m.member_id
WHERE b.deleted = 0 AND m.deleted = 0;
...
**deleted 컬럼 정의:**
```sql
deleted TINYINT DEFAULT 0  -- 0: 활성, 1: 삭제
```
...
모든 테이블에 `created_at`과 `updated_at` 컬럼을 추가하여 **변경 이력을 추적**합니다.
...
| 원칙 | 구현 방식 | 효과 |
|:---:|:---|:---|
| **정규화** | FK로 관계 설정, 중복 제거 | 데이터 일관성 ✅ |
| **권한 관리** | role 컬럼 | 접근 제어 ✅ |
| **데이터 보존** | Soft Delete | 이력 관리 ✅ |
| **변경 추적** | 타임스탐프 | 감시 기능 ✅ |
...
### 시나리오: 회원이 닉네임을 변경하는 경우

**Before (나쁜 설계):**
```
1. 게시글 테이블에 "김철수"라고 저장됨
2. 회원이 닉네임을 "이영희"로 변경
3. 게시글의 작성자명은 여전히 "김철수"로 표시됨 ❌
4. 모든 게시글을 수동으로 업데이트해야 함 (비효율)
```

**After (좋은 설계):**
```
1. 게시글 테이블에 member_id = 5만 저장
2. 회원이 닉네임을 "이영희"로 변경
3. 게시글 조회 시 JOIN으로 자동으로 "이영희" 표시됨 ✅
4. 수동 업데이트 불필요 (효율적)
```
...
이 설계는 다음을 고려하여 결정했습니다:

1. **데이터 무결성**: 정규화를 통한 데이터 일관성 보장
2. **유지보수성**: 변경사항이 자동으로 반영되어 관리 비용 감소
3. **확장성**: 새로운 기능 추가 시 기존 구조에 영향 최소화
4. **실무 적용성**: 실제 서비스에서 사용하는 패턴 적용
